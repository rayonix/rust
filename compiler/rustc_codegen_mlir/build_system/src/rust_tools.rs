...
impl RustcTools {
    fn new(command: &str) -> Result<Option<Self>, String> {
        ...
        // We run this script from "build_system/target/release/build_system", so we need to remove these elements.
        for to_remove in &["build_system", "release", "target", "build_system"] {
            if parent_dir.last().map(|part| part == to_remove).unwrap_or(false) {
                parent_dir.pop();
            } else {
                return Err(format!(
                    "Build script not executed from `build_system/target/release/build_system` (in path {})",
                    current_exe.display(),
                ));
            }
        }
        ...
        if toolchain_version != default_version {
            println!(
                "rustc_codegen_mlir is built for {} but the default rustc version is {}.",
