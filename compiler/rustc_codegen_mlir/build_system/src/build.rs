...
pub fn build_sysroot(env: &HashMap<String, String>, config: &ConfigInfo) -> Result<(), String> {
    ...
    // sent to rustc_codegen_mlir and not the LLVM backend.
...
fn build_codegen(args: &mut BuildArg) -> Result<(), String> {
    ...
    // Setup MLIR path for building
    let mlir_path =
        args.config_info.mlir_path.clone().expect(
            "The config module should have emitted an error if the MLIR path wasn't provided",
        );
    env.insert("LD_LIBRARY_PATH".to_string(), format!("{}/lib", mlir_path));
    env.insert("LIBRARY_PATH".to_string(), format!("{}/lib", mlir_path));
    env.insert("MLIR_PATH".to_string(), mlir_path);
    ...
    let mlir_target = "target/out/mlir";
    create_dir(mlir_target)?;
